...
pub trait InputFile {
    ...
    fn from_default() -> Result<Self, Error> where Self: Sized {
        Self::from_file(format!("{}/{}", Self::DEFAULT_DIRECTORY, Self::DEFAULT_FILE_NAME))
    }

    fn from_file<P: AsRef<Path>>(path: P) -> Result<Self, Error> where Self: Sized;
